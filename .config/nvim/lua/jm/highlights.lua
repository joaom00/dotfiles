...
---@return string
function jm.highlight.alter_color(color, percent)
...
function jm.highlight.get(group, attribute, fallback)
...
---@return ErrorMsg[]?
function jm.highlight.set(namespace, name, opts)
  ...
  if not clear then
    ...
    for attr, value in pairs(opts) do
      if type(value) == "table" and value.from then
        local new_attr, err = jm.highlight.get(value.from, value.attr or attr)
        if value.alter then
          new_attr = jm.highlight.alter_color(new_attr, value.alter)
        end
        if err then
...
function jm.highlight.set_winhl(name, win_id, hls)
  local namespace = api.nvim_create_namespace(name)
  jm.highlight.all(hls, namespace)
...
function jm.highlight.all(hls, namespace)
  local errors = jm.fold(function(errors, hl)
    local curr_errors = jm.highlight.set(namespace or 0, next(hl))
    if curr_errors then
      vim.list_extend(errors, curr_errors)
    end
    return errors
  end, hls)
  if #errors > 0 then
    notify(
      jm.fold(function(acc, err)
        return acc .. "\n" .. err.msg
      end, errors, ""),
      "error"
    )
...
function jm.highlight.plugin(name, opts)
  ...
  jm.highlight.all(opts)
  jm.augroup(fmt("%sHighlightOverrides", name), {
    event = "ColorScheme",
    command = function()
