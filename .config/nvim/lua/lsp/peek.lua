local M = {floating_buf = nil, floating_win = nil, prev_result = nil}

local function create_floating_file(location, opts)
  vim.validate {location = {location, "t"}, opts = {opts, "t", true}}
  ...
  local contents = vim.api.nvim_buf_get_lines(bufnr, range.start.line, math.min(
                                                  range["end"].line + 1 + (opts.context or 10),
                                                  range.start.line + (opts.max_height or 15)), -- Don't let the window be more that 15 lines long(height)
                                              false)
  ...
  vim.api.nvim_command("autocmd QuitPre <buffer> ++nested ++once lua pcall(vim.api.nvim_win_close, " .. winnr ..
                           ", true)")
...
local function preview_location_callback(_, method, result)
  if result == nil or vim.tbl_isempty(result) then
    JM.notify("Peek: No location found: " .. method, "info", "Peek")
    return nil
  end

  local opts = {border = "rounded", context = 10}
...
function M.open_file()
  ...
  if not filepath then
    JM.notify("Peek: Unable to open the file!", "info", "Peek")
    return
  end
...
function M.Peek(what)
  ...
  if vim.tbl_contains(vim.api.nvim_list_wins(), M.floating_win) then
    ...
    if not success_1 then
      JM.notify("Peek: You cannot edit the current file in a preview!", "info", "Pee")
      return
    end
    ...
    vim.api.nvim_buf_set_keymap(M.floating_buf, "n", "<CR>", ":lua require('lsp.peek').open_file()<CR>",
    ...
    if not success then
      JM.notify("Peek: Error calling LSP method \"textDocument/" .. what ..
                    "\". The current language lsp might not support it.", "error", "Peek")
    end
  end
end
