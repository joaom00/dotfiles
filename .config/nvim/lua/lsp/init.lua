...
function M.get_ls_capabilities(client_id)
  ...
  if not client_id then error "Unable to determine client_id" end

  client = vim.lsp.get_client_by_id(tonumber(client_id))

  local enabled_caps = {}

  for k, v in pairs(client.resolved_capabilities) do if v == true then table.insert(enabled_caps, k) end end
...
function M.common_on_init(client, bufnr)
  ...
  if not vim.tbl_isempty(formatters) and formatters[1]["exe"] ~= nil and formatters[1].exe ~= "" then
    ...
    JM.notify(string.format("Overriding language server \"%s\" with format provider \"%s\"", client.name,
                            formatters[1].exe), "info", "LSP")
  end
end
...
function M.setup(lang)
  ...
  if lsp_utils.is_client_active(lsp.provider) then return end

  local overrides = JM.lsp.override
  if type(overrides) == "table" then if vim.tbl_contains(overrides, lang) then return end end

  if lsp.provider ~= nil and lsp.provider ~= "" then
    local lspconfig = require "lspconfig"

    if not lsp.setup.on_attach then lsp.setup.on_attach = M.common_on_attach end
    if not lsp.setup.on_init then lsp.setup.on_init = M.common_on_init end
    if not lsp.setup.capabilities then lsp.setup.capabilities = M.common_capabilities() end
