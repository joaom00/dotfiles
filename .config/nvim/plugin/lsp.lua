...
local function augroup_factory(bufnr, client, events)
  return function(feature, commands)
    ...
    if not provider or client.server_capabilities[provider] then
      events[name].group_id = jm.augroup(fmt("LspCommands_%d_%s", bufnr, name), commands(provider))
...
local function setup_mappings(_, bufnr)
  local function with_desc(desc)
    return { buffer = bufnr, desc = fmt("lsp: %s", desc) }
  end

  map("n", "<c-p>", function()
    vim.diagnostic.goto_prev { float = true }
  end, with_desc "go to prev diagnostic")
  map("n", "<c-n>", function()
    vim.diagnostic.goto_next { float = true }
  end, with_desc "go to next diagnostic")

  map({ "n", "x" }, "<space>ca", lsp.buf.code_action, with_desc "code action")
  ...
  map("n", "gK", lsp.buf.signature_help, with_desc "signature help")
...
local function setup_semantic_tokens(client, bufnr)
  ...
  jm.augroup(fmt("LspSemanticTokens%s", client.name), {
...
do
  ...
  local function smart_quickfix_diagnostics()
    ...
    if not jm.is_vim_list_open() and id then
    ...
    id = id
      or api.nvim_create_autocmd("DiagnosticChanged", {
        callback = function()
          ...
          if not jm.is_vim_list_open() or fn.getqflist({ title = 0 }).title ~= TITLE then
            return
          end
          diagnostic.setqflist { open = false, title = TITLE }
...
local function max_diagnostic(callback)
  return function(_, bufnr, diagnostics, opts)
    local max_severity_per_line = jm.fold(function(diag_map, d)
