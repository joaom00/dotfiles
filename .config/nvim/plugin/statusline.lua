...
local icons, lsp, highlight, decorations = jm.ui.icons, jm.ui.lsp, jm.highlight, jm.ui.decorations
local api, fn, fs, fmt = vim.api, vim.fn, vim.fs, string.format
local P = jm.ui.palette
...
local function special_buffers(ctx)
  ...
  if normal_term then
    return "Terminal(" .. fn.fnamemodify(vim.env.SHELL, ":t") .. ")"
  end
  if ctx.preview then
    return "preview"
  end
...
---@param path string
---@return string
local function path_sep(path)
  return not jm.empty(path) and path .. sep or path
end
...
local function filename(ctx)
  ...
  if jm.empty(parent) then
    return { fname = fname }
  end

  -- local dir = path_sep(table.concat(parts, sep))
  -- local _, env = dir_env(dir)
  -- local segment = not jm.empty(env) and env .. new_dir or dir
  -- if api.nvim_strwidth(segment) > math.floor(vim.o.columns / 3) then
  --   dir = fn.pathshorten(dir)
  -- end

  return { parent = path_sep(parent), fname = fname }
end
...
local function stl_file(ctx, minimal)
  ...
  local env_empty, dir_empty, parent_empty = jm.empty(p.env), jm.empty(p.dir), jm.empty(p.parent)
...
local function diagnostic_info(context)
  ...
  local lsp_icons = jm.ui.icons.lsp
  local result = {
  ...
  return jm.fold(function(accum, item)
...
function jm.ui.statusline.lsp_client_click()
...
---@return string
function jm.ui.statusline.render()
  ...
  local lsp_clients = jm.map(function(client, index)
    return component(client.name, hls.client, {
      ...
      click = "v:lua.jm.ui.statusline.lsp_client_click",
...
local function adopt_window_highlights()
  ...
  if jm.empty(curr_winhl) or not curr_winhl.StatusLine then
    return
  end
  ...
  for _, part in pairs(stl_winhl) do
    ...
    if not jm.empty(hl) then
      return
    end
    highlight.set(name, { inherit = part.fallback, background = { from = curr_winhl.StatusLine, attr = "bg" } })
  end
end
...
jm.augroup("CustomStatusline", {
  ...
  command = function(args)
    set_stl_ft_icon_hls(args.buf, args.match)
  end,
}, {
  event = "WinEnter",
...
-- set the statusline
vim.o.statusline = "%{%v:lua.jm.ui.statusline.render()%}"
